...
| Transcript: return to input | At the bottom, `↓` selects the input; `Enter` enables editing without sending |
...
| Copy selected text | `Ctrl+C`, `F3` or `Ctrl+Shift+C` |
...
Settings offers Warm sand, Midnight, Forest and Paper palettes, accent swatches, language, compact layout, output following and the new-session directory. Colors preview immediately; Cancel restores the previous appearance. Language applies on Save. User messages have a separate gray background; changing the interface language does not translate user messages, model replies, paths or server data.
...
Text copy uses the system clipboard and also sends OSC 52 for compatible terminals. macOS uses its built-in clipboard tools; Windows uses PowerShell; Linux needs `wl-clipboard` for Wayland or `xclip` for X11. Image clipboard access uses Pillow and the available desktop clipboard. Headless/SSH environments may have no desktop clipboard. Use **F4** to attach a readable image file when clipboard access is unavailable or the terminal intercepts Ctrl+V.
