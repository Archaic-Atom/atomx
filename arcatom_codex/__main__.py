...
def main():
    parser = argparse.ArgumentParser(description=tr('Arcatom Codex · 轻量终端会话工作台'))
    parser.add_argument("--cwd", default=str(Path.cwd()), help=tr('新会话的默认工作目录'))
    parser.add_argument("--codex", default="codex", help=tr('Codex 可执行文件'))
    parser.add_argument("--demo", action="store_true", help=tr('离线演示，不连接 Codex、不消耗用量'))
    parser.add_argument("--check", action="store_true", help=tr('只验证本机接口，不发送模型请求'))
