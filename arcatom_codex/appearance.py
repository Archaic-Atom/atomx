...
ACCENT_COLORS = {
    "copper": ("#e5a277", "#905031"),
    "blue": ("#82becb", "#256579"),
    "green": ("#9dc39a", "#3b713d"),
    "violet": ("#bdacf0", "#6d48a0"),
    "rose": ("#e8a0b1", "#993f58"),
}
...
def palette_for(preferences: dict) -> Palette:
    ...
    palette = PALETTES.get(
        preferences.get("ui_theme", "gray"), PALETTES["gray"]
    )
    colors = ACCENT_COLORS.get(preferences.get("accent", "copper"))
    return (
        replace(palette, accent=colors[0 if palette.dark else 1])
        if colors
        else palette
    )
...
def user_message_style(palette: Palette) -> str:
    ...
    return (
        "bold #e6e6e6 on #373737" if palette.dark else "bold #292d30 on #dcdcd9"
    )
