...
class CommandActions:
    ...
    async def native_command(self, command, tid):
        ...
        if self.demo:
            self.show_text("原生 Codex · 演示模式", command + " 会在真实模式下打开官方交互界面。\n演示模式不会启动真实客户端或修改你的配置。")
            return
        if any(s.active_turn or s.busy_since or s.status == "运行中" for s in self.store.sessions.values()) or self.sending:
            raise RpcError("有任务正在运行。请等待完成或停止任务，再打开原生命令。")
        if not sys.stdin.isatty() or not sys.stdout.isatty():
            raise RpcError("原生命令需要交互终端，请在 Terminal 或 iTerm 中运行 arcatom。")
        session = self.store.get(tid) if tid else None
        if session and session.meta.get("ephemeral"):
            raise RpcError("临时分支无法交给另一个进程恢复；请先用 /fork 创建普通分支，再打开原生命令。")
        ...
        for loaded in list(self.store.sessions.values()):
            if loaded.resumed:
                ...
                if terminals.get("data"):
                    raise RpcError("仍有后台终端运行，请先用 /ps 查看并在需要时 /stop，再进入原生命令。")
        ...
        try:
            with self.suspend():
                result = subprocess.run(argv, cwd=str(Path(__file__).resolve().parents[1]), check=False)
            if result.returncode:
                self.notify(f"官方 Codex 已退出（{result.returncode}），已返回 Arcatom。", severity="warning")
        finally:
    ...
    async def select_model(self, tid, argument="", effort_only=False):
        ...
        if not models:
            raise RpcError("后端没有返回可选模型，请检查模型提供商配置。")
        current = session.meta.get("model")
        if effort_only:
            model = next((m for m in models if m["model"] == current), None)
            if model is None:
                raise RpcError("当前模型不在后端目录中，请先用 /model 选择模型。")
        else:
            target = argument.strip() or await self.choose(
                "选择模型 · 当前会话", [(m["model"], m.get("displayName", m["model"]) + "  " + m.get("description", "")) for m in models], current)
            if target is None:
                return
            model = next((m for m in models if m["model"] == target or m["id"] == target), None)
            if model is None:
                raise RpcError("模型不在当前账户的可选目录中：" + target)
        ...
        if efforts:
            effort = await self.choose("推理强度 · " + model.get("displayName", model["model"]),
        ...
        self.command_notice(tid, "已选择 " + model["model"] + (" · " + effort if effort else "") + "，下一回合生效。")
    ...
    async def slash(self, text):
        ...
        self.hide_commands()
        try:
            if not self.ready and name not in ("help", "exit", "quit", "warnings", "theme"):
    ...
    async def dispatch_command(self, name, argument, tid):
        ...
        if name in ("model", "reasoning"):
            ...
            cwd = await self.push_screen_wait(NewSession(session.meta.get("cwd", self.cwd) if session else self.cwd))
            if cwd:
            ...
            target = await self.choose("当前会话的子代理", [(i, s.get("name") or self.store.get(i).title) for i, s in session.agents.items()])
            if target:
            ...
            await self.ensure_resumed(tid)
            self.show_text("当前会话", {"threadId": tid, **session.meta, "tokenUsage": session.usage})
        elif name == "rename":
            title = argument or await self.push_screen_wait(Prompt("会话名称", session.title))
            if title:
                await self.client.call("thread/name/set", {"threadId": tid, "name": title})
                session.meta["name"] = title
        elif name in ("pwd", "cwd", "rollout"):
            self.show_text("会话路径", session.meta.get("path" if name == "rollout" else "cwd") or "后端未提供路径")
        elif name == "cd":
            value = argument or await self.push_screen_wait(Prompt("工作目录", session.meta.get("cwd", self.cwd)))
            if value:
            ...
            self.command_notice(tid, "已切换到" + ("计划模式" if mode == "plan" else "执行模式"))
            if argument:
            ...
            self.show_text("任务目标 · /goal 目标 · pause / resume / clear", result)
        elif name in ("archive", "delete"):
            if session.active_turn or tid in self.sending:
                raise RpcError("请先按 Ctrl+C 停止当前任务。")
            ...
            self.current = None
            self.show_home()
            self.notify("会话已" + ("删除" if name == "delete" else "归档"))
        elif name == "ps":
            ...
            await self.refresh_activity(tid)
            self.command_notice(tid, "已停止当前会话的后台终端。Ctrl+C 可停止模型回合。")
        elif name == "skills":
            ...
            async for page in self.client.pages("mcpServerStatus/list", {}):
                data.extend(page)
            self.show_text("MCP 服务与工具", data or "没有配置 MCP 服务")
        elif name == "debug-config":
            result = await self.client.call("config/read", {"cwd": session.meta.get("cwd", self.cwd), "includeLayers": True})
            self.show_text("配置来源", redact(result))
        elif name == "warnings":
            self.show_text("诊断信息", "\n".join(self.store.account_errors + list(getattr(self.client, "stderr", []))) or "当前没有诊断警告。")
        elif name in ("recap", "init"):
            await self.send_command_prompt(tid, "请总结当前会话的目标、已完成工作、关键决策和待办事项。" if name == "recap" else
                "请检查当前项目并创建适用的 AGENTS.md，记录项目结构、构建测试命令和开发约定；如果文件已存在，先阅读并保留已有有效规则。")
        elif name == "copy":
            ...
            if not text:
                raise RpcError("当前会话还没有完整回复可复制。")
            self.copy_to_clipboard(text)
            self.notify("已发送到终端剪贴板。")
        elif name == "export":
            ...
            self.transcript_signature = None
            self.transcript_cache.clear()
            self.command_notice(tid, "纯文本显示" + ("已开启" if self.raw_transcript else "已关闭"))
        elif name == "theme":
            from pygments.styles import get_all_styles
            value = await self.choose("代码语法高亮主题", [(s, s) for s in sorted(get_all_styles())], self.code_theme)
            if value:
            ...
            value = await self.push_screen_wait(Prompt("状态栏顺序 · time tokens context limits model · 留空字段会隐藏", " ".join(self.status_fields)))
            if value:
                fields = value.split()
                if len(set(fields)) != len(fields) or any(f not in ("time", "tokens", "context", "limits", "model") for f in fields):
                    raise RpcError("字段应为 time tokens context limits model，不可重复。")
                self.status_fields = fields
                self.view_preferences["status_fields"] = fields
                await self.save_view_preferences()
        elif name == "title":
            value = await self.choose("终端标题", [("app", "Arcatom Codex"), ("session", "会话名称"), ("model", "模型 · 会话名称")])
            if value:
                self.view_preferences["title"] = value
                await self.save_view_preferences()
        elif name == "help":
            self.show_text("命令与快捷键", "输入 / 显示提示；↑↓ 选择，Tab 补全，Enter 执行，Esc 收起。\n← 空输入返回列表；Ctrl+X 删除选中历史；Ctrl+C 停止回合。\n带‘原生’的命令会打开官方 Codex，退出后回到 Arcatom。\n\n" +
                "\n".join(f"/{c.name:24} {c.description}" + ("  [原生]" if c.native else "") for c in COMMANDS))
    ...
    async def select_skill(self, session):
        ...
        target = await self.choose("Skills · 选择后输入任务", [(s.name, f"${s.name}  {s.description}\n{s.path}") for s in skills])
        if target:
    ...
    async def export_chat(self, session, argument):
        ...
        value = argument or await self.push_screen_wait(Prompt("导出 Markdown 文件路径", default))
        if not value:
            return
        path = Path(value).expanduser()
        if not path.is_absolute():
        ...
        self.command_notice(session.id, "已导出：" + str(path))
    ...
    async def git_diff(self, session):
        ...
        for filename in untracked.split(b"\0"):
            ...
            if len(diff) > 2_000_000:
                ...
                break
        self.show_text("Git 修改", diff.decode(errors="replace") or "工作区没有修改。")
    ...
    async def mention_file(self, session, argument):
        ...
        if argument:
            ...
            selected = await self.choose("选择文件 · 输入筛选", [(p, p) for p in paths])
            if not selected:
                return
            value = str(root / selected)
        self.query_one(Composer).load_text("请查看文件 " + json.dumps(value, ensure_ascii=False) + " ")
