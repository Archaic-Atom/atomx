...
def server_socket(binary: str) -> Path:
    ...
    if directory.is_symlink() or directory.stat().st_uid != os.getuid():
        raise RuntimeError("AtomX runtime directory has an unexpected owner.")
...
def ensure_local_server(binary: str, cwd: str | None) -> str:
    ...
    def ready() -> bool:
        """Probe the private socket without starting a model turn.
    ...
    with endpoint.with_suffix(".lock").open("a") as lock:
        ...
        if endpoint.exists():
            if not endpoint.is_socket():
                raise RuntimeError("Unexpected file at the AtomX socket path.")
            endpoint.unlink()
