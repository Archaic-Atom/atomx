"""Offline protocol fixtures for demonstrations and tests. 用于演示与测试的离线协议数据。"""
...
class DemoClient:
    """In-memory protocol fixture without account access. 不访问账户的内存演示后端。"""

    def __init__(self) -> None:
        """Initialize local state without sending model requests. 初始化本地状态。"""
        self.events: asyncio.Queue[dict] = asyncio.Queue()
        self.calls: list[tuple[str, dict]] = []
        self.replies: list[tuple[Any, dict]] = []
        ...
        self.threads["demo-login"]["turns"] = [
            {
                ...
                "items": [
                    ...
                    },
                    {
                        ...
                        "type": "agentMessage",
                        "text": (
                            "我已把检查分给两个子代理。\n\n后端已确认 token 刷新逻辑；前端正在"
                            "检查登录后的跳转。你可以在下方展开 **代理与进程** 查看详情。"
                        ),
                    },
                    {
                        "id": "collab1",
                        ...
                        "tool": "spawnAgent",
                        "status": "completed",
                        "senderThreadId": "demo-login",
                        "receiverThreadIds": ["demo-backend", "demo-frontend"],
                        "agentsStates": {
                            "demo-backend": {"status": "completed"},
                            "demo-frontend": {"status": "running"},
                        },
                    },
                    {
                        "id": "command1",
                    ...
                    },
                    {
                        ...
                        "status": "inProgress",
                        "processId": "321",
                        "aggregatedOutput": "Local: http://localhost:5173/\nReady in 280ms",
                    },
                ],
            }
        ]
        for tid, name in [
            ...
            self.threads[tid] = {
                ...
                "cwd": "~/Projects/atlas",
                "updatedAt": now,
                "parentThreadId": "demo-login",
                "source": {"subAgent": "other"},
                "status": {
                    "type": "idle" if tid == "demo-backend" else "active"
                },
                "turns": [
                    {
                        "id": "child-turn",
    ...
    async def call(
        ...
        if method == "thread/list":
            ...
            threads = [
                ...
                if t.get("parentThreadId") == parent
            ]
            return {
                "data": [
                    {k: copy.deepcopy(v) for k, v in t.items() if k != "turns"}
                    for t in threads
                ],
                "nextCursor": None,
            }
        if method == "thread/items/list":
            entries = [
                {"item": copy.deepcopy(item), "turnId": turn["id"]}
        ...
        if method == "permissionProfile/list":
            return {
                "data": [
                    {"id": "read-only", "allowed": True, "description": "只读"},
                    {
                        "id": "workspace-write",
                        "allowed": True,
                        "description": "项目内读写",
        ...
        if method in ("thread/compact/start", "review/start"):
            self.tasks.append(
                asyncio.create_task(
                    self.respond(params["threadId"], "演示：" + method)
                )
            )
            return {}
        if method == "thread/fork":
            ...
            return {
                "thread": copy.deepcopy(meta),
                "model": meta.get("model", "demo-codex"),
            }
        ...
        if method.startswith("thread/goal/"):
            ...
            if method.endswith("/set"):
                meta.setdefault("goal", {}).update(
                    {k: v for k, v in params.items() if k != "threadId"}
                )
        ...
        if method in ("thread/read", "thread/resume"):
            ...
            if params.get("excludeTurns") or (
                method == "thread/read" and not params.get("includeTurns")
            ):
                metadata.pop("turns", None)
            return {
                "thread": metadata,
                "model": thread.get("model")
                if thread.get("model") in ("demo-codex", "demo-reasoner")
                else "demo-codex",
                "reasoningEffort": thread.get("reasoningEffort", "medium"),
                "collaborationMode": thread.get("collaborationMode"),
            }
        ...
        if method == "thread/backgroundTerminals/list":
            return {
                ...
                if params["threadId"] == "demo-login"
                else []
            }
        if method == "account/usage/read":
            return {
                "summary": {"lifetimeTokens": 1248300},
                "dailyUsageBuckets": [
                    {"startDate": "演示当天", "tokens": 42180}
                ],
            }
        ...
        if method == "thread/delete":
            ...
            while True:
                children = {
                    ...
                    if meta.get("parentThreadId") in ids
                }
                if children <= ids:
                    break
                ids.update(children)
    ...
    async def reply(self, rid: Any, result: dict) -> None:
        """Answer one pending backend request. 回复后端待处理请求。"""
        self.replies.append((rid, result))

    async def reject_unsupported(self, rid: Any) -> None:
        """Reject a request that this client cannot safely handle. 拒绝未支持的请求。"""
        self.replies.append((rid, {"error": "unsupported"}))

    async def respond(
        self, tid: str, content: str, client_id: str | None = None
    ) -> None:
        """Emit a synthetic response for the offline demonstration.

        离线演示生成模拟回复。
        """

        async def emit(method: str, **params: Any) -> None:
            """Queue a synthetic backend notification. 将模拟通知放入队列。"""
            await self.events.put(
                {"method": method, "params": dict(threadId=tid, **params)}
            )

        uid, aid = str(uuid.uuid4()), str(uuid.uuid4())
        await emit(
            "turn/started", turn={"id": "demo-response", "status": "inProgress"}
        )
        await emit(
            "item/completed",
            item={
        ...
        await emit(
            "item/started", item={"id": aid, "type": "agentMessage", "text": ""}
        )
        text = (
            "这是离线演示回复。实际模式会把你的消息发送给本机 Codex，并实时显示"
            "回复、子代理和命令状态。"
        )
        for i in range(0, len(text), 5):
            await asyncio.sleep(0.04)
            await emit(
                "item/agentMessage/delta", itemId=aid, delta=text[i : i + 5]
            )
        await emit(
            "item/completed",
            item={"id": aid, "type": "agentMessage", "text": text},
        )
        await emit(
            "thread/tokenUsage/updated",
            turnId="demo-response",
            tokenUsage={
                "total": {
                    "totalTokens": 28100,
                    "inputTokens": 25000,
                ...
                "last": {"totalTokens": 8100},
                "modelContextWindow": 128000,
            },
        )
        await emit(
            "turn/completed",
            turn={"id": "demo-response", "status": "completed"},
        )
