...
def clipboard_command(write: bool) -> list[str] | None:
    ...
    if sys.platform == "win32":
        script = ("$t=[Console]::In.ReadToEnd(); Set-Clipboard -Value $t" if write else
                  "[Console]::OutputEncoding=[Text.UTF8Encoding]::new(); Get-Clipboard -Raw")
        prefix = "[Console]::InputEncoding=[Text.UTF8Encoding]::new(); "
        return ["powershell.exe", "-NoProfile", "-NonInteractive", "-STA", "-Command", prefix + script]
    candidates = ([(["wl-copy"], "wl-copy"), (["xclip", "-selection", "clipboard"], "xclip"),
                   (["xsel", "--clipboard", "--input"], "xsel")] if write else
                  [(["wl-paste", "--no-newline", "--type", "text"], "wl-paste"),
                   (["xclip", "-selection", "clipboard", "-o"], "xclip"),
                   (["xsel", "--clipboard", "--output"], "xsel")])
    return next((argv for argv, binary in candidates if shutil.which(binary)), None)
...
def copy_text(text: str) -> bool:
    ...
    if not argv:
        return False
    try:
        subprocess.run(argv, input=text.encode("utf-8"), stdout=subprocess.DEVNULL,
...
def read_clipboard(cache: Path) -> ClipboardContent:
    ...
    except (OSError, RuntimeError, NotImplementedError, subprocess.SubprocessError):
        image = None
    ...
    if argv:
        try:
            ...
            return ClipboardContent(text=result.stdout.decode("utf-8").rstrip("\r\n")
                                    if sys.platform == "win32" else result.stdout.decode("utf-8"))
